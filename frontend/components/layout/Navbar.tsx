...
export function Navbar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[999] px-4 md:px-6 pt-3 pointer-events-none">
      {/* Progressive Flowing Blur Background (with horizontal and vertical soft masking) */}
      <div 
        className="absolute top-0 left-0 right-0 h-24 pointer-events-none select-none z-[-1]"
        style={{
      ...
      >
        ...
        <div className="absolute inset-0" style={{ backdropFilter: 'blur(24px) saturate(180%) brightness(120%)', WebkitBackdropFilter: 'blur(24px) saturate(180%) brightness(120%)', maskImage: 'linear-gradient(to bottom, black 0%, black 65%, transparent 85%)', WebkitMaskImage: 'linear-gradient(to bottom, black 0%, black 65%, transparent 85%)' }} />
        
        {/* Soft fading blur outside the nav bar */}
        <div className="absolute inset-0" style={{ backdropFilter: 'blur(8px) saturate(140%)', WebkitBackdropFilter: 'blur(8px) saturate(140%)', maskImage: 'linear-gradient(to bottom, transparent 65%, black 85%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom, transparent 65%, black 85%, transparent 100%)' }} />
        
        {/* Very subtle transition edge */}
        <div className="absolute inset-0" style={{ backdropFilter: 'blur(2px)', WebkitBackdropFilter: 'blur(2px)', maskImage: 'linear-gradient(to bottom, transparent 85%, black 95%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom, transparent 85%, black 95%, transparent 100%)' }} />
