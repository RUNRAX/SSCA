...
Before ANY tool call, query graphify_ssca first.
Call mcp_graphify_ssca_query_graph even if the task is small.
...
4. Call `mcp_graphify_ssca_query_graph(<next step context only>)`
...
4. Call `mcp_graphify_ssca_query_graph()` for full codebase context
